...
async function launchWithFallback() {
  if (process.env.STARDUST_FORCE_HEADED === '1') {
    const headed = await launchHeadedStealth();
    return { browser: headed, technique: 'headed-chrome-stealth-forced' };
  }
...
async function newContext(browser, stealth, storageState) {
  const ctx = await browser.newContext({ ...CRAWL_CONTEXT, ...(storageState ? { storageState } : {}) });
  if (stealth) {
...
function isChallengeResponse(resp) {
  ...
  if (status === 403 || status === 429 || status === 503) {
    ...
    // PerimeterX / HUMAN Security behind Fastly/Varnish (1800contacts.com):
    // 403 block page sets a _pxhd cookie and serves via Varnish.
    const setCookie = (h['set-cookie'] || '');
    if (setCookie.includes('_pxhd') || setCookie.includes('_px3')) return true;
    if (server.includes('varnish') && h['retry-after'] === '0') return true;
...
async function capturePage(context, url, slug, args) {
  ...
  // Interstitial hygiene (1800contacts finding): the verification modal is a
  // cosmetic overlay on an already fully-served page (HTTP 200, full DOM) —
  // prune it from the capture exactly like the consent containers above, so
  // the screenshot shows the page, not the overlay. This does not interact
  // with the challenge; it only removes the overlay nodes from the snapshot.
  await page.evaluate(() => {
    document.querySelectorAll('#px-captcha-wrapper, [id^="px-captcha"]').forEach((n) => {
      const dlg = n.closest('[role="dialog"], [class*="modal" i], [class*="overlay" i]');
      (dlg || n).remove();
    });
  }).catch(() => {});
  // park the pointer so no :hover state is captured after dismissal clicks
...
async function main() {
  ...
  let stealth = process.env.STARDUST_FORCE_HEADED === '1';
  ...
  let ok = 0;
  // clone the probe context's storage state (bot-management clearance cookies,
  // e.g. PerimeterX _px3/_pxhd) into every worker context — per extract SKILL.md
  // Setup step 3, cookies do not cross contexts and each fresh context would be
  // re-challenged otherwise.
  const seededState = await context.storageState().catch(() => null);
  await context.close();
  ...
  async function worker() {
    const ctx = await newContext(browser, stealth, seededState);
